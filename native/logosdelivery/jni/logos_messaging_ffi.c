...
// Relay peer count for the given pubsub topic ("" = all). Result message is
// the number as a decimal string.
jobject Java_com_receiverandroid_LogosMessagingModule_wakuNumConnectedPeers(JNIEnv *env, jobject thiz,
                                                jlong wakuPtr,
                                                jstring pubsubTopic) {
  cb_result *result = NULL;
  const char *topic = (*env)->GetStringUTFChars(env, pubsubTopic, 0);
  waku_relay_get_num_connected_peers((void *)wakuPtr, on_response, (void *)&result, topic);
  jobject response = to_jni_result(env, result);
  free_cb_result(result);
  (*env)->ReleaseStringUTFChars(env, pubsubTopic, topic);
  return response;
