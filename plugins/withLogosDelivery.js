...
// 5) iOS — register the local LogosMessaging pod (embeds liblogosdelivery.a).
//    Only when the static lib has actually been built; without it the pod is
//    skipped and iOS falls back to the js-waku backend.
const withIosPod = (config) =>
  withDangerousMod(config, [
    "ios",
    async (cfg) => {
      const root = cfg.modRequest.projectRoot;
      const podDir = path.join(root, "native", "logosdelivery", "ios");
      if (!fs.existsSync(path.join(podDir, "liblogosdelivery.a"))) {
        console.warn(
          "withLogosDelivery: native/logosdelivery/ios/liblogosdelivery.a not present — " +
            "skipping iOS native node (js-waku backend will be used)."
        );
        return cfg;
      }
      const podfile = path.join(cfg.modRequest.platformProjectRoot, "Podfile");
      let src = fs.readFileSync(podfile, "utf8");
      if (!src.includes("LogosMessaging")) {
        src = src.replace(
          /use_expo_modules!/,
          `use_expo_modules!\n  # Embedded Logos Delivery node (native module, mirrors Android)\n  pod 'LogosMessaging', :path => '../native/logosdelivery/ios'`
        );
...
module.exports = (config) =>
  withIosPod(withLegacyJniPackaging(withGson(withPackageRegistered(withNativeFiles(config)))));
